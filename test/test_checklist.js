...
const CASES = [
  ...
  { practice: 'server-controlled HTML sink is raised to HIGH', insecure: { 'renderer.js': `const res = await fetch('/doc');\nel.innerHTML = await res.text();` }, expect: [{ id: 'XSS_SINK_JS_CHECK', severity: 'HIGH', match: /server-controlled/ }] },
  ...
  { practice: 'regression: inline scripts in HTML files are analyzed (Joplin 2.8.8 note viewer)', insecure: { 'gui/note-viewer/index.html': `<html>\n<body>\n<div id="content"></div>\n<script>\n  ipcProxySendToHost('ready');\n  window.addEventListener('message', (event) => {\n    document.getElementById('content').innerHTML = event.data.html;\n  });\n</script>\n</body>\n</html>` },
    // HTML from a message event is content another window/user controls: raised to HIGH (the stored-content threat)
    expect: [{ id: 'XSS_SINK_JS_CHECK', severity: 'HIGH' }],
