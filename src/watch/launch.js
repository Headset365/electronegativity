...
export function resolveApp(target, extraArgs = []) {
  ...
  if (stat.isDirectory() && resolved.endsWith('.app')) { // macOS bundle
    ...
    return { command: path.join(macos, executable), args: extraArgs, staticInput: exists(asar) ? asar : undefined };
  }
  if (stat.isDirectory()) {
    ...
    return { command: electron, args: [resolved, ...extraArgs], staticInput: resolved };
  }
  const resources = path.join(path.dirname(resolved), 'resources');
  const asar = path.join(resources, 'app.asar');
  const unpacked = path.join(resources, 'app');
  return { command: resolved, args: extraArgs, staticInput: exists(asar) ? asar : exists(unpacked) ? unpacked : undefined };
}
 ...
 */
export function watchApp(target, { args = [], log = path.join(fs.mkdtempSync(path.join(os.tmpdir(), 'electronegativity-watch-')), 'session.jsonl'), stdio = 'inherit' } = {}) {
